...
// Add timestamps to all existing use cases
document.getElementById('add-timestamps-btn').addEventListener('click', async () => {
    if (!confirm('This will add creation and modification timestamps to ALL use cases that don\'t already have them. Continue?')) {
        return;
    }

    const button = document.getElementById('add-timestamps-btn');
    ...
    try {
        ...
        if (snapshot.empty) {
            alert('No use cases found in database.');
        ...
        if (updateCount > 0) {
            await batch.commit();
            alert(`Success! Added timestamps to ${updateCount} use case(s).`);
            loadUseCasesEditor(); // Reload the list
        } else {
            alert('All use cases already have timestamps.');
        }
        ...
        alert('Error adding timestamps: ' + error.message);
        button.disabled = false;
        button.textContent = originalText;
    }
});
